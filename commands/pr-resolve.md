...
gh pr view <PR-URL> --json comments \
  --jq ".comments[] | select(.author.login != \"$SONAR_BOT\") | {author: .author.login, body: .body}"

# Review comments inline (por linha de código)
...
3. Commit atômico:
   ...
   ```
4. Responda o comentário:
   ```bash
   gh pr comment <PR-URL> --body "Resolvido em <hash>. <breve explicação>"
   ```

### Pergunta/dúvida:
```bash
gh pr comment <PR-URL> --body "<resposta clara e direta>"
```

### Ambíguo:
```bash
gh pr comment <PR-URL> --body "Poderia detalhar melhor o que espera nesse ponto? <dúvida específica>"
```
Notifique no terminal: **"Comentário ambíguo. Aguardando clarificação."**
Volte ao polling da Etapa 3.

### Sugestão opcional:
Avalie se agrega valor sem risco. Se sim, implemente. Se não, responda explicando a decisão.
